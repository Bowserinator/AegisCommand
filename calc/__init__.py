...
def formula_as_file(args,user="",hostmask="",extra={}):
    """latex <equation> - Render LaTeX
    {"category":"calc","name":"latex"}"""
    if args.replace(" ","") == "": 
...
def phraseTextMath(text,user="",hostmask="",extra={}):
    ...
    {"category":"calc","name":"calc"}"""
    showMode = "DEFAULT"
    trigMode = "RAD"
    showPrec = 10
    if "DEG" in text: text = text.replace("DEG",""); trigMode = "DEG"
    if "GRAD" in text: text = text.replace("GRAD",""); trigMode = "GRAD"
    for i in ["SCI","FRACT"]:
        if i in text:
    ...
    options = {
        ...
        "trig":trigMode,
        "display":showMode,
        "display_prec":showPrec,
        "max_display_prec":100,
    }
    return "\x02Result: \x0f" + pycalc.phraseTextMath(text,options)[:200]
